...
# Load CORS from environment variables
cors_origins = parse_cors_origins(os.getenv("CORS_ORIGINS"))
allow_all_origins = cors_origins == ["*"]

# If no CORS origins configured, use safe defaults for development
if not cors_origins:
...
if limiter:
    ...
    if rate_limit_exception_handler:
...
MAX_FILE_SIZE_MB = float(os.getenv("MAX_FILE_SIZE_MB", "50"))  # Default 50MB per file
MAX_TOTAL_SIZE_MB = float(os.getenv("MAX_TOTAL_SIZE_MB", "200"))  # Default 200MB total
...
def load_resources() -> None:
    ...
    global anomaly_models, anomaly_scalers
    global trial_feature_medians
    ...
    trial_feature_medians = cast(dict[str, float], trial_df[trial_feature_columns].median(numeric_only=True).fillna(0.0).to_dict())
...
def parse_uploaded_files(files: list[UploadFile]) -> tuple[dict[str, pd.DataFrame], dict[str, Any]]:
    ...
    # Validate file count (not too many files)
    if len(files) > 20:
    ...
    if len(files) == 1 and files[0].filename and files[0].filename.lower().endswith(".zip"):
        ...
        with zipfile.ZipFile(uploaded.file, "r") as archive:
            ...
            missing = [name for name in [*REQUIRED_FILES.keys(), METADATA_FILE] if name not in names]
            if missing:
            ...
            for file_name, sensor_name in REQUIRED_FILES.items():
                df = pd.read_csv(BytesIO(archive.read(names[file_name])))
                validate_csv_content(df, sensor_name)
                sensor_frames[sensor_name] = df

            metadata = json.loads(archive.read(names[METADATA_FILE]).decode("utf-8"))
            
            # Validate required metadata fields
            required_metadata_fields = ["participant_id", "trial_id"]
    ...
    missing = [name for name in [*REQUIRED_FILES.keys(), METADATA_FILE] if name not in uploads_by_name]
    if missing:
...
def validate_csv_content(df: pd.DataFrame, sensor_name: str) -> None:
    """Validate CSV content for potential malicious data."""
    ...
    for col in df.columns:
        ...
        for pattern in suspicious_patterns:
            if pattern in col_str:
                raise HTTPException(status_code=400, detail=f"Invalid column name detected in {sensor_name} file.")
    
    # Check for excessive number of columns (potential attack)
    if len(df.columns) > 100:
        raise HTTPException(status_code=400, detail=f"{sensor_name} file has too many columns (possible attack).")
    
    # Check for excessive row count
    if len(df) > 100000:
        raise HTTPException(status_code=400, detail=f"{sensor_name} file has too many rows (possible attack).")
...
def normalize_sensor_dataframe(sensor_name: str, df: pd.DataFrame, sampling_rate: float) -> pd.DataFrame:
    ...
    candidate_map = {
        f"{prefix}acc_x": "acc_x",
        f"{prefix}acc_y": "acc_y",
        f"{prefix}acc_z": "acc_z",
        f"{prefix}gyr_x": "gyr_x",
        f"{prefix}gyr_y": "gyr_y",
        f"{prefix}gyr_z": "gyr_z",
        f"{prefix}mag_x": "mag_x",
        f"{prefix}mag_y": "mag_y",
        f"{prefix}mag_z": "mag_z",
        "acc_x": "acc_x",
        "acc_y": "acc_y",
        "acc_z": "acc_z",
        "gyr_x": "gyr_x",
...
def build_metadata_graph(prediction: dict[str, Any], anomaly_status: str, metadata: dict[str, Any]) -> dict[str, float]:
    """Build numeric graph values from metadata for visualization."""
    ...
    trials = 1  # currently always 1
    trials_score = min(100, trials * 20)
...
def build_response(
    ...
    return {
        ...
        # Unique per-request token forces frontend to treat every
        # response as new data, preventing stale-graph rendering.
...
# Apply rate limiting decorator if slowapi is available
if limiter:
    predict_fall_risk = limiter.limit("10/minute")(predict_fall_risk)

@app.post("/predict")
async def predict_fall_risk(files: list[UploadFile] = File(...)) -> JSONResponse:
    try:
        ...
        if logger == "DEBUG":
            ...
            raise HTTPException(status_code=500, detail="Processing failed. Please verify uploaded files and try again.") from exc
