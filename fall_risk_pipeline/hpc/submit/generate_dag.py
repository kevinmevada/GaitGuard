...
def write_dag(config_path: Path, output: Path, *, include_downstream: bool) -> None:
    ...
    if pre_jobs:
        lines.append(f"PARENT merge_ingest CHILD {' '.join(pre_jobs)}")
        lines.append(f"PARENT {' '.join(pre_jobs)} CHILD merge_preprocess")
    else:
        lines.append("PARENT merge_ingest CHILD merge_preprocess")
    ...
    if feat_jobs:
        lines.append(f"PARENT merge_preprocess CHILD {' '.join(feat_jobs)}")
        lines.append(f"PARENT {' '.join(feat_jobs)} CHILD merge_features")
    else:
        lines.append("PARENT merge_preprocess CHILD merge_features")
    ...
    if anom_jobs:
        lines.append(f"PARENT merge_features CHILD {' '.join(anom_jobs)}")
        lines.append(f"PARENT {' '.join(anom_jobs)} CHILD reduce_anomaly")
    else:
        lines.append("PARENT merge_features CHILD reduce_anomaly")
